...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createOrUpdateCard(device) {
        ...
        const deviceName = device.deviceName || device.id; // Use device.id as fallback, not streamKey
        card.querySelector('.device-name').textContent = deviceName;
        card.querySelector('.device-id').textContent = `${t('deviceIdLabel')}: ${device.id}`;
    ...
    async function editDeviceName(id) {
        ...
        const currentName = device.deviceName || device.id;
